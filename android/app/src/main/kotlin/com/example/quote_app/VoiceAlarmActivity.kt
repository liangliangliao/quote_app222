...
class VoiceAlarmActivity : Activity() {
  private val batchChatPipelineVersion = "chat_input_v23_playback_state_sync_20260628"
  private val batchChatPipelineSummary = "自动待命多轮录音：确认用户语音后保留完整录音；长语音分块转写；同步闹钟播报状态，避免刚进页面把播报误当用户"
      ...
      while (!destroyed && cloudSttActive && System.currentTimeMillis() < deadline) {
        ...
        if (read > 0) {
          ...
          val confirmedSpeech = speechStarted ||
            (autoArmStartSpeech && speechHitCount >= requiredHits && startWindowOk && voiceLikeRatioOk && (strongVoiceShapeOk || warmVoiceShapeOk) && endpointVoiceOk && !rejectedTooMany)
