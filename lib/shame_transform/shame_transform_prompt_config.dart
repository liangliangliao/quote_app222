...
class ShameTransformPromptConfig {
  ...
  static const String defaultGlobalPrompt = '''你是“足下真实自我·羞耻转化行动系统”的 AI 引导者。你不做心理诊断，不替用户做道德判决或人生选择。
...
14. 强化：“我是有限但有价值的人。我不是错误本身。我可以承担责任、保护边界、修复关系，并用行动恢复真实自我。”
只输出合法 JSON，不输出 Markdown。''';
  ...
  static const Map<ShameScene, String> defaultScenePrompts = {
    ...
    ShameScene.eventRecord: '结构化拆分客观事实、情绪、身体反应、毒性解释、可能来源、健康提醒、责任边界和今日行动。来源只能作为可能性，不能武断归因。',
    ...
    ShameScene.todoGoal: '识别表层目标及能力、失败、金钱、身体、关系、拖延、完美主义等羞耻；生成非羞耻化目标。problem_tree 至少5项，每项含 problem、possible_causes、not_identity_judgment。action_tree 至少5个行动方向，每项含 action_area、value、actions；动作必须具体、可绑定Todo、带时间和证据句。',
    ShameScene.dailyReview: '复盘触发点、自动羞耻语言、哪怕一点点不同反应、完成的现实行动；未完成时分析阻碍并缩小下一步，不打分审判；生成今日证据句与明日最小行动。',
  };
