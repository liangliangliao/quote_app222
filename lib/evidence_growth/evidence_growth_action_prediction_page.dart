    ...
    return _section(
        ...
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ExpansionTile(
              ...
              title: const Text('IBM 理论变量（可选）'),
              subtitle: const Text('不知道就留空；这些条件来自行为预测理论，不再是随意拼出的因素'),
              children: [
                _singleChoice('行动意向：你是否已经真正决定要做？',
                    commitmentStrengthOptions, commitmentStrength,
                    (v) => setState(() => commitmentStrength = v),
                    helper: 'IBM：intention，最接近真实行为的心理决定因素'),
                _chipGroup('体验性态度：想到执行时的直接感受',
                    emotionOptions, emotions,
                    helper: 'IBM：experiential attitude'),
                _singleChoice('工具性态度：从结果和代价看，这件事值得做吗？',
                    attitudeOptions, instrumentalAttitude,
                    (v) => setState(() => instrumentalAttitude = v),
                    helper: 'IBM：instrumental attitude'),
                _singleChoice('命令性规范：重要他人希望你怎么做？',
                    normOptions, injunctiveNorm,
                    (v) => setState(() => injunctiveNorm = v),
                    helper: 'IBM：injunctive norm'),
                _singleChoice('描述性规范：重要他人实际上通常怎么做？',
                    descriptiveNormOptions, descriptiveNorm,
                    (v) => setState(() => descriptiveNorm = v),
                    helper: 'IBM：descriptive norm'),
                _singleChoice('自我效能：你相信自己能完成吗？',
                    efficacyOptions, selfEfficacy,
                    (v) => setState(() => selfEfficacy = v),
                    helper: 'IBM：self-efficacy'),
                _singleChoice('知觉行为控制：你觉得这件事有多受自己控制？',
                    controlOptions, perceivedControl,
                    (v) => setState(() => perceivedControl = v),
                    helper: 'IBM：perceived control'),
                _singleChoice('知识与技能是否足够？',
                    knowledgeSkillOptions, knowledgeSkills,
                    (v) => setState(() => knowledgeSkills = v),
                    helper: 'IBM：knowledge & skills，直接影响行为'),
                _singleChoice('关键时刻这件事会不会保持在注意中？',
                    valueSalienceOptions, valueSalience,
                    (v) => setState(() => valueSalience = v),
                    helper: 'IBM：salience，直接影响意向能否转成行为'),
                _chipGroup('环境／现实约束：已经知道的客观条件',
                    feasibilityOptions, feasibility,
                    helper: 'IBM：environmental constraints'),
                _chipGroup('时间约束', timeCapacityOptions, timeCapacity),
                _chipGroup('身体／精力约束', physicalStateOptions, physicalState),
                _chipGroup('其他现实摩擦', frictionOptions, frictions),
                _singleChoice('过去真正相似的行为通常怎样？',
                    historyOptions, historyPattern,
                    (v) => setState(() => historyPattern = v),
                    helper: 'IBM：habit / past behavior，只看真正同类行为'),
                _chipGroup('执行意图／启动线索',
                    supportOptions, executionSupport,
                    helper: '扩展：implementation intention，例如“当X发生，我就立即做Y”'),
                _singleChoice('已经决定之后，到关键时刻还会重新打开决定吗？',
                    stabilityOptions, decisionStability,
                    (v) => setState(() => decisionStability = v),
                    helper: '这是 intention 稳定性的补充证据，不单独当作新理论构念'),
                _chipGroup('社会责任／外部承诺补充',
                    commitmentOptions, commitments),
                _chipGroup('可能竞争的自动替代行为',
