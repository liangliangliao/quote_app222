...
class _EvidenceGrowthReminderPageState extends State<EvidenceGrowthReminderPage> with WidgetsBindingObserver {
  ...
  Future<void> _load() async {
    ...
    repeatHours = int.tryParse(await widget.dao.getSetting('missing_repeat_hours', fallback:'$hours')) ?? hours;
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body:loading ? const Center(child:CircularProgressIndicator()) : ListView(padding:const EdgeInsets.all(16),children:[
      ...
      if (widget.trialId == null) DropdownButtonFormField<int>(initialValue:hours,
        decoration:const InputDecoration(labelText:'窗口到期后，等待多久首次提醒？'),
      ...
      if (widget.trialId == null) DropdownButtonFormField<int>(initialValue:repeatHours,
        decoration:const InputDecoration(labelText:'仍未反馈时，每隔多久继续提醒？'),
        items:const [1,6,24,48,168].map((h)=>DropdownMenuItem(value:h,child:Text('$h 小时'))).toList(),
        onChanged:busy ? null : (h) async {
          if (h == null) return;
          setState(()=>busy=true);
          try { await widget.dao.configureReminders(repeatHours:h); await _load(); }
          finally { if (mounted) setState(()=>busy=false); }
        }),
      const SizedBox(height:12),
      Text('观察窗口结束后等待 $hours 小时首次提醒；仍未反馈，每隔 $repeatHours 小时继续提醒，直到记录结果或关闭提醒。打开页面或划掉通知不会停止追踪。错过多次时只补一条，随后恢复间隔提醒。'),
      ...
      ...records.map((row) {
        ...
        return Card(child:ExpansionTile(title:Text('${row['title']}'),
          ...
          children:[Padding(padding:const EdgeInsets.all(16),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
            ...
            if (row['kind'] == 'missing_result') const Text('缺反馈连续追踪：每次发送后自动安排下一次；提交真实结果后停止。以本条计划时间为准。'),
