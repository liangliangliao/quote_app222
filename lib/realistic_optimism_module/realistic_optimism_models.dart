...
class RealisticOptimismCase {
  ...
  static RealisticOptimismCase fromJson(Map<String, dynamic> m) {
    ...
    if (_str(m['module']) == 'realistic_optimism' || m.containsKey('intensity_check')) {
      final intensity = _map(m['intensity_check']);
      final emotion = _map(m['emotion_validation']);
      final fact = _map(m['fact_layer']);
      ...
      final action = _map(m['process_action_plan']);
      ...
      final title = _str(m['user_event_summary'], original.isEmpty ? '今日事件重构' : original);
      final balanced = _str(benefit['balanced_interpretation']);
      final fiveMinute = _str(action['five_minute_action']);
      final ifThen = _stringList(action['if_then_plan']);
      final cards = <Map<String, dynamic>>[
        <String, dynamic>{'title': '强度分级', 'content': '${_str(intensity['level'])} · ${_str(intensity['reason'])}', 'type': 'safety'},
        ...
        <String, dynamic>{'title': '5分钟行动', 'content': fiveMinute, 'type': 'action'},
        <String, dynamic>{'title': '身份提醒', 'content': _str(identity['identity_sentence']), 'type': 'identity'},
      ];
      return RealisticOptimismCase(
        ...
        coreProblem: _str(emotion['validation_text'], _str(m['final_user_message'])),
        mainBelief: _str(style['automatic_interpretation']),
        riskLevel: _str(intensity['level'], 'L2'),
        aiPosition: '现实主义乐观：承认痛苦，分离事实与解释，再回到可控行动。',
        oldBelief: _str(style['automatic_interpretation']),
        facts: _stringList(fact['objective_facts']),
        interpretations: <String>[
          if (_str(style['automatic_interpretation']).isNotEmpty) _str(style['automatic_interpretation']),
          if (_str(style['main_pattern']).isNotEmpty) '主要模式：${_str(style['main_pattern'])}',
          '永久化 ${_int(style['permanence_score'])}/10 · 普遍化 ${_int(style['pervasiveness_score'])}/10 · 人格化 ${_int(style['personalization_score'])}/10',
          '灾难化 ${_int(style['catastrophizing_score'])}/10 · 无力化 ${_int(style['helplessness_score'])}/10 · 过滤化 ${_int(style['filtering_score'])}/10',
        ],
        hiddenAssumptions: _stringList(fact['unknowns_or_assumptions']),
        ...
        probabilityImprovers: _stringList(benefit['possible_learning']),
        neededState: _str(prime['daily_value_word']),
        positiveCues: <String>[_str(prime['lock_screen_sentence']), _str(prime['benefit_finder_question'])].where((e) => e.trim().isNotEmpty).toList(),
        phonePrompt: _str(prime['lock_screen_sentence']),
        environmentChanges: <String>[_str(prime['benefit_finder_question'])].where((e) => e.trim().isNotEmpty).toList(),
        antiPrimingCleanup: <String>[_str(prime['anti_prime_cleanup_action'])].where((e) => e.trim().isNotEmpty).toList(),
        todayMinimumAction: fiveMinute,
        actionTime: '今天最早可用的 5 分钟',
        actionPlace: '当前能降低干扰的位置',
        minimumSuccessStandard: '完成一个行动证据，而不是完成全部问题。',
        ifThenPlan: ifThen.join('\n'),
        possibleObstacle: _str(style['main_pattern']),
        fallbackAction: fiveMinute,
        ...
        feedbackValue: _str(failure['psychological_antibody']),
        nextAdjustment: _stringList(action['next_three_steps']).join('\n'),
        avoidPattern: _str(fault['likely_behavioral_effect']),
        comfortZoneAction: '先允许情绪存在 2 分钟。',
        stretchZoneAction: fiveMinute,
        panicZoneAction: '强迫自己立刻积极或一次解决全部问题。',
        recommendedAction: fiveMinute,
        selfPerceptionAfterAction: _str(identity['identity_sentence']),
        options: _stringList(action['next_three_steps']),
        recommendedButNotForced: fiveMinute,
        reflectionQuestion: _str(gratitude['savoring_prompt']),
        displayCards: cards,
