    ...
    return RefreshIndicator(
      ...
      child: ListView(padding: const EdgeInsets.fromLTRB(16, 14, 16, 90), children: [
        ...
        if (_entries.isEmpty)
          ...
          ..._entries.take(7).map((e) => Card(child: ListTile(leading: Icon(e.returnedAfterBreak ? Icons.replay_circle_filled : Icons.directions_walk, color: e.returnedAfterBreak ? Colors.green : _goalBlue), title: Text('${e.effortTypeLabel} · ${e.effortMinutes} 分钟 · ${e.zoneLabel}'), subtitle: Text('${e.investmentText}${e.smallProgress.trim().isEmpty ? '' : '\n小进步：${e.smallProgress}'}${e.returnedAfterBreak ? '\n我又回来了：这次不是从零开始。' : ''}'))),
      ]),
    );
  }
}
