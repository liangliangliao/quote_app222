...
请把固定型、羞耻型身份评价改写成努力语言。只围绕用户原话回答三点：可观察事实、可能的系统变量、一个 2-10 分钟可验证调整。不得诊断人格，不得空泛鼓励，不得说教。输出 2-4 句自然中文，不要 JSON。''',
  ...
  Future<String> generateEffortResponsePaper({required List<TodoGoalEffortEntry> entries, required List<TodoGoalRitual> rituals}) async {
    ...
    final state = await getGlobalAiState();
    final evidence = entries.take(30).map((e) => '${e.effortDate}｜${e.goalTitle}｜${e.effortMinutes}分钟｜${e.zoneLabel}｜类型:${e.effortTypeLabel}｜对象:${e.contextLabel}｜阶段:${e.cycleStageLabel}｜投入:${e.investmentText}｜阻力:${e.frictionLabel}｜策略:${e.strategyUsed}｜学习:${e.timeInLearning}｜感激:${e.gratitudeText}｜回归:${e.returnedAfterBreak ? '${e.recoveryGapHours}小时' : '否'}').join('\n');
    final ritualText = rituals.take(10).map((r) => '${r.goalTitle}｜触发:${r.triggerText}｜行动:${r.minimumAction}｜预案:${r.frictionPlan}｜稳定度:${r.stabilityScore}%｜启动:${r.completedCheckIns}/${r.checkInCount}').join('\n');
    String fallback() {
