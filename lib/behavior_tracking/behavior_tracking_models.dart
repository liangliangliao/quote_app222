...
class BehaviorTrackingTemplate {
  ...
  static String templateKeyForMode(String mode) {
    switch (mode) {
      ...
      case 'weekly_review':
        return 'weekly_review_default';
      case 'monthly_review':
        return 'monthly_review_default';
      default:
...
const behaviorTrackingTemplates = <BehaviorTrackingTemplate>[
  ...
  BehaviorTrackingTemplate(
    key: 'weekly_review_default',
    name: '周复盘',
    purpose: '把近 7 天的时间分布、高频触发、替代动作和下一周实验收束成闭环。',
    defaultEntryMode: 'weekly_review',
    fieldKeys: ['time_waste', 'trigger_text', 'long_term_impact', 'alternative', 'tomorrow_adjustment'],
    completionRules: ['require_pattern_and_next_action'],
  ),
  BehaviorTrackingTemplate(
    key: 'monthly_review_default',
    name: '月复盘',
    purpose: '比较 30/90 天趋势，识别即时奖励但长期亏损的行为。',
    defaultEntryMode: 'monthly_review',
    fieldKeys: ['important_behaviors', 'time_waste', 'mood', 'long_term_impact', 'tomorrow_adjustment'],
    completionRules: ['require_trend_and_one_experiment'],
  ),
];
