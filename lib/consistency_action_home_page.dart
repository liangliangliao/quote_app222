...
const String _consistencySystemPrompt = '''你是“足下一致行动系统”的 AI 引导者。你的核心任务不是替用户做决定，也不是用强迫、羞耻、夸大激励或外部奖励控制用户，而是基于认知失调理论与“强迫服从的认知后果”研究思想，帮助用户通过自主选择的最小行动，逐步建立新的自我证据，并让行动反向塑造更稳定、更真实的信念与身份。
...
const String _consistencyOutputPrompt = '''请严格按照以下格式输出：
【一、你现在的核心冲突】
【二、这不是失败，而是一个改变入口】
【三、你的1美元行动】
【四、行动前承诺语】
...
$_consistencyOutputPrompt''';
    setState(() => _aiBusy = true);
    try {
      final text = await _ai.generateText(
        prompt: prompt,
        systemPrompt: _consistencySystemPrompt,
        purpose: 'consistency_action.$scene',
        maxTokens: 1400,
        temperature: 0.4,
      );
      if (!mounted) return;
      setState(() => _analysis = text.trim().isEmpty ? _analysis : text.trim());
    } catch (e) {
      if (!mounted) return;
...
下周建议身份方向：${weekly.isEmpty ? '先制造第一条真实证据' : weekly.first.identity}''';
  ...
  Widget _buildCompass() => _PageShell(children: [
        ...
        _ChoiceWrap(values: _coreValues, selected: _selectedValues, onChanged: (v) => setState(() => _selectedValues.contains(v) ? _selectedValues.remove(v) : _selectedValues.add(v))),
  ...
  Widget _buildLedger() => _PageShell(children: [
        ...
        for (final group in _groupedEvidence().entries) _Section(title: group.key, body: group.value.map((e) => '• ${e.action}\n  新解释：${e.newStory}\n  身份：${e.identity}').join('\n\n')),
      ]);
  ...
  Widget _buildRewardDenoise() => _PageShell(children: [
        ...
        _ChoiceWrap(values: _rewardOptions, selected: _selectedRewards, onChanged: (v) => setState(() => _selectedRewards.contains(v) ? _selectedRewards.remove(v) : _selectedRewards.add(v))),
        _Section(title: '当前外部理由评估', body: _rewardRiskText()),
      ]);

  Widget _buildRationalization() => _PageShell(children: const [
        ...
        _Section(title: '修复性小行动', body: '写下一句更诚实但不羞辱的解释：我现在确实有困难，但我仍然可以做一个 30 秒的小动作来靠近价值。'),
      ]);
